...
func main() {
	...
	if err != nil {
		...
		if len(aResponse.Answers) > 0 {
			for _, answer := range aResponse.Answers {
				...
				// 你也可以进行类型断言来访问特定类型的字段
				// if aRec, ok := answer.Body.(*dnsmessage.AResource); ok {
				// 	fmt.Printf("  IP地址: %s\n", aRec.A)
				// }
			}
		} else {
	...
	if err != nil {
		...
		// fmt.Println("原始响应:", aaaaResponse) // 可以取消注释查看完整响应
		fmt.Println("解析结果 (Answer Section):")
	...
	if err != nil {
		...
		// fmt.Println("原始响应:", mxResponse) // 可以取消注释查看完整响应
		fmt.Println("解析结果 (Answer Section):")
	...
	if err != nil {
		// 注意：对于 NXDOMAIN，查询本身可能成功，但 RCode 会指示错误
