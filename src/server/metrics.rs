...
use prometheus::{
    ...
    register_int_counter_vec, register_int_gauge_vec, register_int_counter
};
...
impl DnsMetrics {
    ...
    pub fn new() -> Self {
        ...
        let http_request_duration_seconds = register_histogram_vec!(
            opts!("owdns_http_request_duration_seconds", "HTTP request processing duration in seconds, classified by method, path and format"),
            &["method", "path", "format"],
            registry
        ).unwrap();
        
        let http_request_bytes = register_histogram_vec!(
            opts!("owdns_http_request_bytes", "HTTP request size in bytes, classified by method and path"),
            &["method", "path"],
            registry
        ).unwrap();
        
        let http_response_bytes = register_histogram_vec!(
            opts!("owdns_http_response_bytes", "HTTP response size in bytes, classified by method and path"),
            &["method", "path"],
            registry
        ).unwrap();
        ...
        let cache_ttl_seconds = register_histogram_vec!(
            opts!("owdns_cache_ttl_seconds", "TTL distribution of DNS cache entries in seconds"),
            &[],
            registry
        ).unwrap();
        ...
        let dns_query_duration_seconds = register_histogram_vec!(
            opts!("owdns_dns_query_duration_seconds", "DNS query processing duration in seconds, classified by query type"),
            &["query_type"],
            registry
        ).unwrap();
        ...
        let upstream_duration_seconds = register_histogram_vec!(
            opts!("owdns_upstream_duration_seconds", "Upstream query duration in seconds, classified by resolver address, protocol and upstream group"),
            &["resolver", "protocol", "upstream_group"],
            registry
        ).unwrap();
        ...
        let cache_persist_duration_seconds = register_histogram_vec!(
            opts!("owdns_cache_persist_duration_seconds", "Cache persistence operation duration in seconds, classified by operation type (save, load)"),
            &["operation"],
            registry
        ).unwrap();
